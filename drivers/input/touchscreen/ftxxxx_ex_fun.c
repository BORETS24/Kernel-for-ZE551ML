...
//#endif
//<ASUS_DTP->
static ssize_t ftxxxx_swipe_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	//struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
static ssize_t ftxxxx_swipe_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	//struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
...
//#endif
//<ASUS_DTP->
static DEVICE_ATTR(ftsswipemode, S_IRUGO|S_IWUSR, ftxxxx_swipe_mode_show, ftxxxx_swipe_mode_store);
//#endif
