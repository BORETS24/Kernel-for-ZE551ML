...
static void check_gesture(struct ftxxxx_ts_data *data,int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_SWIPE:
			...
			input_report_key(data->input_dev, KEY_GESTURE_SWIPE_UP, 1);
			//input_sync(data->input_dev);
			input_report_key(data->input_dev, KEY_GESTURE_SWIPE_UP, 0);
			input_sync(data->input_dev);
			}                                  
			break;
		//case GESTURE_DOWN:
		...
		case GESTURE_DOUBLECLICK:
			...
			if (dclick_mode == 1) {
				...
				input_report_key(data->input_dev, KEY_GESTURE_DCLICK, 1);
				input_report_key(data->input_dev, KEY_GESTURE_DCLICK, 0);
				input_sync(data->input_dev);
			}
			break;
		//case GESTURE_O:
...
//<ASUS_Proximity->
	...
	//<ASUS_i2c_suspend+>
	if ((dclick_mode == 1) || (swipe_mode == 1) || (gesture_mode & 0x40)) {
	...
	if(state ==1 && ((dclick_mode == 1) || (swipe_mode == 1) || (gesture_mode & 0x40)))
	{
...
static int ftxxxx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_DCLICK);
	...
	//__set_bit(KEY_GESTURE_SWIPE_UP, input_dev->keybit);
	//__set_bit(KEY_GESTURE_DOWN, input_dev->keybit);
	//__set_bit(KEY_GESTURE_U, input_dev->keybit);
	//__set_bit(KEY_GESTURE_O, input_dev->keybit);
	__set_bit(KEY_GESTURE_E, input_dev->keybit);
	__set_bit(KEY_GESTURE_DCLICK, input_dev->keybit);
	__set_bit(KEY_GESTURE_SWIPE_UP, input_dev->keybit);
